...
public class SysMenuController extends AbstractController {
	...
	@RequestMapping("/nav")
	public R nav() {
		List<NavMenuEntity> navList = sysMenuService.getNavMenuList(getUserId(), getSuperUser());

		NavMenuEntity menu = new NavMenuEntity();
		menu.setTitle("后台首页");
		menu.setIcon("larry-shouye4");
		menu.setUrl("main.html");

		if (navList.size() > 0) {
			// 追加后台首页到菜单
			@SuppressWarnings("unchecked")
			List<NavMenuEntity> childrenList = (List<NavMenuEntity>) navList.get(0).getChildren();

			childrenList.add(0, menu);
		} else {
			navList.add(menu);
		}
	...
	public List<SysMenu> list(SysMenu sysMenu) {
		...
		for (SysMenu SysMenu : menuList) {
			SysMenu parentMenuEntity = sysMenuService.getSysMenuById(SysMenu.getParentId());
			Map<String, Object> map = Maps.newHashMap();
			map.put("parentName", parentMenuEntity != null ? parentMenuEntity.getName() : null);
			SysMenu.setMap(map);
	...
	private void verifyForm(SysMenu menu) {
		...
		if (menu.getParentId() != 0) {
			SysMenu parentMenu = sysMenuService.getSysMenuById(menu.getParentId());
